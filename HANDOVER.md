...
| **★(신규, 2026-09-01) Bulk package.json Version Checker** | `tools/package-json-version-checker.html` | 검증완료 — node 로직 8케이스 + parseSpec 단위 16케이스 + **jsdom DOM 8시나리오(XSS 포함)** | package.json 여러 개를 `-----`로 구분해 붙여넣으면 **매니페스트끼리 비교**. 외부 라이브러리 0개(내장 JSON 파서 + 자체 semver 파서). `dependencies`/`devDependencies`/`peerDependencies`/`optionalDependencies` 4개 섹션 모두 읽음. **심각도 5단계로 분리**: major(메이저 불일치) / unparsed(workspace:·catalog:·git·npm alias 등 비교 불가) / section(같은 패키지가 매니페스트마다 다른 섹션에) / minor(같은 메이저 내 버전 차이) / range(버전 같고 범위 연산자만 다름). **section 검사는 syncpack엔 있지만 웹 도구엔 없는 항목** — 버전이 아니라서 버전 중심 리뷰를 그대로 통과함. **세션 중 발견/수정한 버그 1건**: 같은 `name`을 가진 매니페스트 2개가 내부 키 충돌로 한 컬럼으로 합쳐져 **불일치를 통째로 놓침** → 매니페스트마다 고유 키를 부여하고 표시용 라벨을 분리. jsdom 안 돌렸으면 "이슈 0"으로 잘못 보고할 뻔했음. 자체 semver 파서는 `^ ~ >= <= > < =`, `v` 프리픽스, 부분 버전(`1`, `1.2`), prerelease, `*`/`latest`를 처리하고 프로토콜 지정자는 `other`로 분류. **명암비 전수 계산 AA 통과**(다크 8.89~15.72:1, 밝은 패널 5.12~11.61:1) |
| **★(신규, 2026-08-25) Bulk JSON Field Consistency Checker** | `tools/json-field-consistency.html` | 검증완료 — node 로직 9케이스 + **jsdom DOM 8시나리오(XSS 이스케이프 포함)** + 200레코드×30필드 성능 53ms | JSON 배열 응답을 그대로 붙여넣거나 `-----`로 구분한 개별 레코드를 넣으면 **레코드끼리 비교**. 외부 라이브러리 0개(내장 JSON 파서만). **검사 6종**: optional(일부에만 존재) / type drift(레코드마다 타입 바뀜) / always-null(전 레코드 null이라 타입 추론 불가) / missing vs null(없는 것과 null인 것 혼재) / empty vs null / **naming drift**. 중첩은 dot path(`meta.region`), 객체 배열은 `items[]`로 축약(6단계 깊이). **차별화의 핵심 = 경쟁 도구가 전부 2개 문서 diff뿐**이고 N개 일관성 분석이 시장에 없음. 타입 생성기(quicktype)는 N개를 읽지만 **출력이 코드라 불일치를 `?`와 union으로 흡수** → 감사 목적엔 정반대. 이 대비를 페이지에 명시함. **명암비 전수 계산 통과**(다크박스 8.89~15.72:1, 밝은패널 5.12~11.61:1, 전부 AA). 34항대로 jsdom 클릭 시뮬레이션 수행 — 푸터 스크립트 누출 없음 확인 |
...
- **14차(2026-08-24 세션, 주간 데이터 세션): 후보 1개 검토 → 기각. 신규 툴 0개.**
  - **Bulk URL Length Checker (기각)**: GSC에서 `url length checker` 24노출/63.5위가 잡혀서 검토했으나 — inetventures.com이 **수백 개 붙여넣기 + CSV export**, tooladex.com이 **브라우저 로컬 실행 명시**, behindthesearch.in이 **20 URL 배치 + 18개 검사 항목**. 배치·붙여넣기·no-upload 3개 필터 전부 탈락. 게다가 **개념 자체가 문자 수 세기라 너무 얕음**(11차에서 HTML Entity Encoder를 같은 이유로 기각한 전례).
  - **이번 주는 신규보다 기존 강화가 명백히 우선이었음** — 8번 8/24 블록의 "신규 페이지도 미크롤링 큐에 들어감" 발견 참고. 억지로 만들지 않음.

- **15차(2026-08-25 세션, "키워드 폭넓게 / GSC 밖에서 / 새 클러스터 환영" 지시): 5개 검토 → 4개 기각, 1개 채택.**
  - **Bulk Redirect Map Generator (기각)**: 사이트 이전용 old→new URL 매핑. static.app이 **.htaccess·Nginx·Netlify·Vercel·IIS·meta refresh 6종 전부 + 브라우저 로컬**, vibetool.org가 **10만 URL 클라이언트사이드 + CSV/JSON export**, seo.domains가 **bulk + nginx map 디렉티브까지**. 3필터 전부 탈락.
  - **Bulk SPF/DMARC/DKIM Checker (기각)**: 새 클러스터(이메일 인증)로 매력적이었고 우리가 DoH 인프라도 있으나 — dmarcgenerator.com이 **"dozens of domains at once + export"**, dmarcreport.com·checkdmarcrecord.com이 무가입 무료. 배치 필터 탈락.
  - **Bulk Alt Text Checker (기각)**: 새 클러스터(접근성)로 heading-structure-checker와 페어링도 좋았으나 — avp.io가 **HTML 붙여넣기 + missing/empty/suspicious 분류(WCAG 1.1.1)**, suparank.io가 **"runs instantly and privately in your browser, nothing sent to a server"**, easyprotools가 배치+붙여넣기+CSV export. 3필터 전부 탈락.
  - **JSON→타입 생성기 계열 (기각)**: quicktype이 다중 샘플을 이미 지원하고 app.quicktype.io는 오프라인 동작까지 명시.
  - **★ Bulk JSON Field Consistency Checker (채택 — 28번째 툴)**: **경쟁 도구가 전부 "2개 문서 diff"뿐**이라는 게 결정적. coderstool·jsondiffs·testmuai·jsonviewertool·onlinejsonformatt 전부 좌/우 2개만 받음. **N개 레코드의 일관성을 보는 도구가 없음.**
    - **결정적 증거**: jsonutils.online 블로그가 우회법으로 **"bash로 10개 응답을 받아 1번과 각각 하나씩 diff하라"**는 루프를 안내함 = 사람들이 실제로 겪는 통증이 문서화돼 있는데 도구가 없는 상태.
    - **타입 생성기와의 차이를 정직하게 서술함**: quicktype류는 N개 샘플을 읽지만 **출력이 코드**라서 불일치를 `email?: string`이나 union으로 **조용히 흡수**함. 감사 목적에는 정확히 반대 동작. 이 대비를 툴 페이지와 블로그 양쪽에 명시.
    - **우리 검증된 패턴의 3번째 적용**: heading-structure-checker(페이지 간), frontmatter-checker(포스트 간)에 이어 **"항목 간 비교라야만 답이 나오는 문제"**. 이게 우리가 반복해서 뚫은 유일한 각도임 — 다음 후보도 이 패턴으로 찾을 것.
    - **네이밍 드리프트 휴리스틱 재사용**: frontmatter에서 검증된 "비슷한 두 이름이 한 레코드에서 절대 같이 안 나오면 같은 필드" 판정을 그대로 적용 → `user_id`/`userId`는 잡고 `title`/`subtitle`은 오탐 없음. API 페이로드에서 snake_case/camelCase 직렬화 경로가 두 개일 때 잡히는 실전 케이스.

- **16차(2026-09-01 세션, "키워드 폭넓게 / GSC 밖에서 / 새 클러스터 환영" 지시): 3개 검토 → 2개 기각, 1개 채택. 39항 렌즈(N개를 함께 봐야만 답이 나오는 문제)로 후보를 좁혀서 조사함.**
  - **CSV 컬럼/헤더 일관성 (기각)**: 우리 CSV 클러스터(Bing에서 csv-to-json 2위)와 인접해 관객 적합성이 가장 좋았으나 — datablist·csvtool.io·piraiai·extendsclass가 전부 **브라우저 로컬 + 컬럼 매핑 + 누락/추가 컬럼 플래그 + diff 리포트 다운로드**까지 구현. N파일 공백은 기술적으로 존재하지만 **2파일(before/after)이 CSV 유스케이스의 대부분을 덮고, 키워드 공간이 이미 밀도 높음.** 무리해서 들어갈 자리가 아님.
  - **JSON→타입 생성기 재검토 (기각, 15차와 동일)**: quicktype 다중 샘플 지원 유지.
  - **★ Bulk package.json Version Checker (채택 — 29번째 툴)**: **웹 도구는 전부 2파일 diff, N파일 도구는 전부 CLI 설치형**이라는 이중 공백.
    - **결정적 증거**: 가장 가까운 웹 경쟁사 comparetext.org(package-json-diff / compare-json)가 **자기 FAQ에서 "한 화면에서는 안 된다. 3개면 쌍으로 나눠 비교하라. diff는 원래 양쪽을 위한 것"이라고 명시**. 경쟁사가 스스로 N파일을 안 하겠다고 선언한 상태.
    - **N파일 진영은 전부 설치형**: syncpack(AWS·Cloudflare·Datadog·Vercel 등이 사용, npm 설치 + 워크스페이스 레이아웃 + 설정파일 필요), manypkg, packcomp(`npm i -g packcomp`로 정확히 N개 package.json 비교). GitHub Gist에 사람들이 직접 스크립트를 짜 올려둔 것도 확인 → **통증은 문서화됐는데 웹 도구가 없음**(15차 JSON 때와 동일한 패턴).
    - **syncpack이 못 덮는 지점이 우리 자리**: syncpack은 **로컬에 체크아웃된 모노레포**가 전제라 별개 repo끼리, 팀원이 붙여넣은 매니페스트, 스타터 템플릿 대조 같은 케이스를 못 함.
    - **새 클러스터**: 의존성/모노레포 위생. repo 전체 grep 결과 package.json·monorepo·semver·npm 언급이 **기존 파일에 0건** — 완전한 신규 영역.

**대기 중인 신규 툴 후보**: 없음 (16차에서 3개 검토 후 2개 기각·1개 채택으로 소진). 다음 세션에서도 "배치 차별화" 우선, 막히면 "no-upload 차별화"(8차), 그마저 없으면 "경쟁사가 전부 단일 항목만 처리하는 헤드키워드"(10차) 순으로 확인하고, **전부 막히면 억지로 만들지 말고 정직하게 기각 보고할 것**(11차에서 확립된 원칙).
...
- **전체 지표**: 클릭 10→**10 (증가 0)**, 노출 3970→**4798(+21%)**, 쿼리 442→498, 노출 페이지 55. 클릭 4건은 전부 지난주와 동일한 쿼리(`rss feed generator`, `screaming frog license free`, `rss generator free`, `rss feed builder free`)에서 나왔고 **8/22 이후 9일간 신규 클릭 0.**
- **★★ 8/18 내부링크 실험 — 3주차 실패 판정.** 고노출 페이지 본문에서 링크를 심은 3개(`sitemap-generator`←858노출, `image-batch`←174, `csv-tsv-converter`←57)가 **전부 그대로 "발견됨-미색인"에 남아 있음.** 대조군이던 `pdf-merge-compress`(링크 못 심음)와 결과가 동일. **즉 "고노출 페이지에서 본문 링크를 걸면 크롤 예산이 흘러간다"는 가설은 기각됐다.**
  - 8/11 lastmod 도입도 이 18개에는 효과 없음(17→18로 오히려 증가, 증가분은 신규 발행분).
  - **결론: 우리가 쓸 수 있는 온사이트 레버(내부링크·lastmod)로는 이 18개를 못 뺀다.** 같은 시도를 반복하지 말 것(41항).
- **★ 신규 페이지 색인은 되고 있다 — 다만 기준을 못 찾겠다(정직 기록).** 8/19 발행분 2개가 **둘 다 색인되어 노출 발생**: `tools/frontmatter-checker.html` 3노출/55.67위, `blog/markdown-front-matter-mistakes.html` 2노출/33위. 그런데 **8/11 발행한 `blog/sitemap-lastmod-git-commit-date.html`은 여전히 미색인.**
  - 링크로 설명이 안 됨: 미색인인 8/11 글은 **순위 12.75위짜리 `sitemap-static-sites`에서 본문 링크를 받고 있고**, 색인된 8/19 글은 갓 만든 신규 툴에서만 링크를 받았다. **링크 가설과 정반대 결과.**
  - 8/25 발행분(`json-field-consistency`, `which-api-fields-are-optional`)은 아직 노출 데이터 없음 — 다음 스냅샷에서 확인할 것.
  - **38항(신규 페이지는 1~2주 대기)은 유효하되, "대기 후 색인된다"는 보장은 없다**는 게 8/11 사례로 확인됨.
- **노출 기저선 유지**: 8/12 이후 하루 104~181로 3주째 유지. 8/22에 181로 최고치. 평균 순위 64~70위대는 하위 페이지가 계속 새로 노출되며 희석된 결과.
- **순위 15~40 구간**: `humans txt generator` 60→**91노출이지만 순위 28.98→39.31로 10계단 하락**(5스냅샷 연속 상승이 처음 꺾임, 상업가치 0이라 조치 안 함), `zapier rss` 15노출/29.33위(**5주 연속 완전 동일**), `bulk meta title and description checker` 5노출/16.40위(3주째 동일), llms.txt 클러스터 18~36위 동일.
- **★ JWT 클러스터에 새 신호**: `blog/jwt-claims-explained.html`이 574노출/0클릭인데, **헤드텀 `jwt claims`는 81.58위인 반면 개별 클레임 쿼리들이 작업 구간에 있음** — `jti jwt` 18.50위, `jwt sub field` 25위, `what are claims in jwt token` 25위, `jwt id` 31위, `jwt payload claims` 32위, `subject claim` 38위. Bing은 더 강함: `what are registered, public, and private claims` **2위**, `registered fields in jwt` 6위, `what are public claims in jwt` 7위, `undefined: jwt.registeredclaims` 8위.
  - **다만 이번엔 조치 안 함**: 해당 페이지(1283단어)를 확인해보니 registered/public/private 3분류와 iss·sub·aud·exp·nbf·iat·jti를 **이미 전부 다루고 있어서 36항(검색의도 불일치)에 해당하지 않음.** 순위가 낮은 건 콘텐츠 공백이 아니라 권위 문제. **다음 세션에서 개별 클레임 쿼리 순위가 더 오르면 그때 재검토.**
- **Bing (4주차)**: 노출 39→**80(2배)**, 클릭 2건 유지. `batch csv to json converter` **2위 / CTR 50%**, `bulk json file validator` **4위 / CTR 100%**.
  - **★ 포지셔닝 검증 신호**: `bulk utility tool` 5노출/**5.4위**, `json file bulk tool` 10위, `bulk uploads and utility diff` 9위, `itself tools batch barcode generator` 5위 — **"bulk/batch 도구"라는 범용 탐색 쿼리에서 우리가 상위에 잡힘.** 우리 배치 포지셔닝이 시장에서 인식되고 있다는 뜻.
  - Google 미색인 페이지(`why-merged-pdf-is-bigger` 3.67위, `yaml-anchors-aliases` 5.2위, `robots-txt-vs-llms-txt` 3위)를 Bing은 계속 상위 색인 중 — 31항 재확인.
- **GA4(8/03~8/30)**: 활성 64명. (direct)/(none) 49명은 봇. **실사용자는 google/organic 3 + bing/organic 2 + duckduckgo 1 + chatgpt.com/ai-assistant 1 + 디렉토리 referral 8 = 약 15명.** 이탈률 홈 67%, 평균 참여 9.5초(지난주 35초에서 급락 — 봇 비중 증가로 보임).
  - **★ `chatgpt.com / ai-assistant` 유입이 2주 연속 발생(1건씩).** 8/24에 처음 나왔을 때 "1건이라 신호 아님"이라고 적었는데 **재현됐으므로 이제 약한 신호로 볼 것.** llms.txt 클러스터 투자 근거가 될 수 있으나 아직 절대량이 작다.
- **수익화**: 9-1-1 조건 여전히 미달(RSS 자동화 클러스터 실클릭 0). 보고에서 제외.

**2026-08-31 세션 조치 내역**:

1. **`blog/excel-csv-number-mangling.html` 보강 (이번 주 유일한 조치, 786→1344단어).** 근거: **구글 29.2위(작업 구간 15~40) + Bing 5.25위**로 양쪽 다 순위가 좋은데 클릭 0이고, **Bing 쿼리들이 본문이 설명하지 않는 메커니즘을 정확히 묻고 있었음** — `why does a csv file changing the numebrs at the end`(2위), `when convert csv to excel the first column with numbers appear 2,50708e+17`(5위), `why is my program treating csv as number instead of text`(9위), `why is my program treating csv as number instead of text`. 36항 전형 케이스.
   - 기존 본문은 "12자리 이상이면 과학적 표기로 바뀌고 원본 숫자가 영구 손실될 수 있다"고만 적혀 있었음 — **모호하고, 복구 가능 여부를 판단할 수 없음.**
   - 추가한 것: (a) **15자리 경계** — Excel이 IEEE 754 배정밀도로 저장해 유효숫자 15자리만 보존하므로 **12~15자리는 표시 문제(복구 가능), 16자리 이상은 16번째부터 0으로 치환되는 실제 데이터 손실(복구 불가)** 로 나뉜다는 것. (b) **피해가 눈에 안 띄는 이유** — 저장 후 다시 열면 증상이 `E+`가 아니라 **"끝이 0으로 끝나는 멀쩡해 보이는 숫자"**여서 아무도 못 알아챔. Bing 2위 쿼리("숫자 끝이 바뀐다")가 정확히 이 증상. (c) **"이미 당했으면 어떻게 하나" 분류표** — 원본 자릿수를 세어 15자리 이하면 서식만 고치면 되고, 16자리 이상이면 Excel 안에서는 방법이 없으니 원본 CSV에서 재임포트. (d) **따옴표는 보호가 안 된다** — CSV의 따옴표는 필드 경계 표시일 뿐 타입 지정이 아니고, 아포스트로피는 Excel 셀 관례라 CSV가 이해하지 못함. 생성 측에서 할 수 있는 게 없다는 것을 명시(`why is my program treating csv as number instead of text` 대응).
   - **경쟁 회피 앵글**: `excel scientific notation csv` 헤드텀은 thebricks(다수 글)·splitforge·w3tutorials·ordoro로 포화. 그래서 **"어떻게 막나"가 아니라 "이미 벌어졌는데 복구 가능한가 — 어떻게 판별하나"**라는 진단·분류 각도만 파고듦. 15자리 경계로 복구 가능/불가를 가르는 판단 기준을 명시한 곳이 드물고, "끝이 0으로 끝나는 증상" 프레이밍은 사실상 없음.
   - meta description도 "15자리"를 전면에 내세우는 문구로 교체.
2. **신규 툴 0개, 신규 글 0개.** 미크롤링 18개가 3주째 그대로이고 8/25 발행분도 아직 노출이 안 잡힌 상태라, **이번 주는 신규 페이지 추가보다 이미 순위가 있는 페이지 강화가 명백히 우선**(38항). 공격적 확장 전략 자체는 유지.

**2026-08-24 세션 — 데이터 재확인 및 조치 사항**:

- **★★ 클릭이 5주 만에 움직였다. 7 → 10.** 7/27 이후 19일간 0이던 클릭이 **8/17, 8/19, 8/21에 각 1건씩** 발생. 노출 3255→3970(+22%), 쿼리 397→442.
  - **클릭 발생 쿼리 4개**: `rss feed generator`(1clk/26imp/70.0위), **`screaming frog license free`(1clk/6imp/33.8위 — 신규)**, `rss generator free`(1clk/5imp/53.6), `rss feed builder free`(1clk/2imp/41.0위 — 신규).
  - **페이지별 신규 클릭 3건**: `blog/free-alternative-screaming-frog.html` 0→**1**(사이트 최대 노출 944), `tools/rss-generator.html` 3→**4**, 홈 2→**3**.
  - **모바일 클릭 첫 발생(1건).** 8/4부터 6개 스냅샷 연속 "모바일 클릭 0"이었던 패턴이 깨짐. 모바일 CTR 0.51%로 데스크톱(0.25%)의 2배.
- **★ 노출 급증이 유지됨 — lastmod 효과 판정 가능.** 8/12부터 하루 104~143을 **10일 연속 유지**(그 이전 2주는 35~59). 일시적 스파이크가 아니라 새 기저선이 됐다. 다만 8/8 자연 감소분과 섞였던 문제가 있으므로 **"lastmod 단독 효과"로 단정하지 말고 "8/11 이후 조치 전체의 누적 효과"로 기록**. 순위는 45~50위대 → 64~69위대로 나빠졌는데, 이건 **하위 순위 페이지가 새로 노출되기 시작해서 평균이 희석된 것**(노출 페이지 수는 53으로 동일하나 쿼리가 45개 늘었음)이라 악화로 볼 근거가 아님.
- **★ 색인 17개 그대로, 신규 1개 추가되어 18개.** 8/18에 심은 본문링크 8개는 **아직 하나도 해소 못 시킴(6일 경과, 2~4주 필요하므로 실패 판정 아님).**
  - **⚠️ 중요 발견 — 신규 페이지도 미크롤링 큐에 들어감.** 8/11에 발행한 `blog/sitemap-lastmod-git-commit-date.html`이 **8/18자로 "발견됨-미색인"에 진입**(최종크롤링 1970-01-01). 즉 **발행 → 발견까지 7일, 그 이후로는 크롤링 대기.** 8/19 발행한 신규 2개(frontmatter-checker, markdown-front-matter-mistakes)도 같은 경로를 탈 가능성이 높음.
  - **전략적 함의(전략 변경 아님, 우선순위 판단용)**: 지금은 **페이지를 새로 늘리는 것보다 이미 색인돼서 노출/클릭이 나오는 페이지를 강화하는 쪽의 ROI가 명확히 높다.** 새 페이지는 최소 1~2주 크롤 대기열에 앉아 있고, 우리 크롤 예산은 이미 포화 상태다. **공격적 확장 전략 자체는 유지하되(사용자 지시), 주차별 우선순위에서는 "클릭 나오는 페이지 강화"를 위에 둘 것.**
- **순위 15~40 구간**: `humans txt generator` 45→**60노출/28.98위**(5스냅샷 연속 상승, 여전히 상업가치 0이라 추적만), `zapier rss` 15노출/29.33위(**4주 연속 완전 동일**), `bulk meta title and description checker` 5노출/16.4위(동일), llms.txt 클러스터 전부 동일.
- **Bing (3주차)**: 총 노출 39건(14→39), **클릭 2건**(csv-to-json 2위/1클릭, json-validator 4위/1클릭). 순위대 2~10위 유지. `github pages sitemap robots static site` 6노출/**2위**, `do llms read llms.txt robots.txt for ai crawlers` 4노출/**3위**. **Google 미색인 페이지(yaml-anchors-aliases 5위, why-merged-pdf-is-bigger 8위, robots-txt-vs-llms-txt 3위)를 Bing은 계속 색인·상위 노출 중** — 8/18 결론(콘텐츠가 아니라 도메인 권위 문제) 재확인.
- **GA4(7/27~8/23)**: 활성 63명. (direct)/(none) 47명 + Council Bluffs 3·Ashburn 2·Frankfurt 4는 봇. **실사용자는 google/organic 4 + bing/organic 2 + duckduckgo 1 + chatgpt.com/ai-assistant 1 + 디렉토리 referral 7 = 15명 수준**(8/18의 7명에서 2배). Busan 5는 사용자 본인. **`chatgpt.com / ai-assistant` 유입 1건은 이번이 처음** — llms.txt 투자와 관련될 수 있으나 1건이라 신호로 보긴 이름, 다음 스냅샷에서 재확인할 것.
  - GA4 페이지 목록에 **아랍어 제목(`بديل مجاني...`)으로 스크리밍프로그 글이 잡힘** = 구글 자동번역 노출. 영어권 밖 수요가 있다는 뜻이지만 별도 대응은 안 함.
- **수익화**: 9-1-1 트래픽 조건 여전히 미달(RSS 자동화 클러스터 실클릭 0). 보고에서 제외.

**2026-08-24 세션 조치 내역**:

1. **`blog/free-alternative-screaming-frog.html` 보강 (이번 주 유일한 조치, 901→1547단어).** 근거: 이 페이지가 **사이트 최대 노출(944)이면서 이번 주 첫 클릭을 만들어낸 페이지**이고, 클릭을 만든 쿼리 `screaming frog license free`가 **순위 33.8위로 작업 구간(15~40) 안**에 있는데, **기존 본문이 스크리밍프로그의 무료 버전·라이선스 얘기를 단 한 줄도 다루지 않고 있었음.** 노출은 있는데 검색 의도와 본문이 어긋나 있던 전형적 케이스.
   - 추가한 것: (a) **무료 버전이 이미 제공하는 것**(500 URL 한도, 깨진링크·메타·중복·사이트맵 export) — "무료판은 반쪽짜리"라는 흔한 오해를 정직하게 부정, (b) **500 한도가 예상보다 빨리 닳는 이유** — 한도가 "페이지"가 아니라 **크롤러가 만나는 모든 URL**(이미지·CSS·JS·PDF·외부링크)을 세기 때문에 120페이지 사이트도 콘텐츠 다 보기 전에 소진됨. 리소스 파일 제외 + 하위폴더 한정으로 완화 가능, (c) 무료판은 **크롤 저장/재개 불가**, (d) FAQ 2개 — "정말 유료가 필요한가"(필요 조건을 구체적으로 나열, 소규모 사이트면 라이선스가 사줄 게 없다고 명시) + **"공짜로 유료 기능 얻는 합법적 방법 있나"(없음 — 무료판이 곧 체험판이고, 비공식 할인/무제한 라이선스는 정당하게 제공될 수 없는 것을 파는 것이며 크랙 빌드를 클라이언트 인프라에 겨누는 건 나쁜 거래라고 명확히 서술)**.
   - **경쟁 회피 앵글**: "screaming frog free vs paid"는 seoraf·searchatlas·vrid·comparedge·getspike 등이 이미 포화. 그래서 **별도 신규 글을 쓰지 않고 기존 글을 보강하는 쪽을 택함** — (1) 자기잠식 방지, (2) 위 미크롤링 문제상 신규 페이지는 최소 1~2주 대기열에 앉음, (3) 경쟁사가 안 다루는 지점은 "500이 페이지 수가 아니라 URL 수"라는 실제 소모 메커니즘이라 이것만 롱테일로 파고듦.
   - **우리 한도도 숫자로 명시(정직성)**: 40페이지/회이고 500보다 훨씬 작다는 것, 사이트가 그보다 크면 **무료 데스크톱 빌드가 더 나은 무료 선택지이며 이 글은 그쪽으로 보내는 게 맞다**고 본문에 적음. 11번 16항(가치없는/부정확한 콘텐츠 금지) 준수.
   - **경쟁사 가격은 29항대로 처리**: 공식 사이트 기준 £199/$279 per year(2026-08 확인)로 적되 **"예산 잡기 전에 현재 금액을 확인하라"**는 문구를 같이 넣어 시점 명시.
   - meta description도 무료판/라이선스 의도에 맞게 교체(기존엔 "eventually paid"라는 모호한 표현이었음).
2. **sitemap.xml**: 위 1개 파일만 lastmod 2026-08-24로 갱신(27항 준수). 최다일 비율 25%로 균일성 문제 없음.
3. **신규 툴 0개** — 14차 후보 검토 결과 기각(5번 참고).
...
36. **(신규, 2026-08-24) "노출은 많은데 클릭이 0인 페이지"는 품질 문제가 아니라 검색의도 불일치일 수 있으니, 그 페이지에 붙은 쿼리를 먼저 볼 것.** `blog/free-alternative-screaming-frog.html`은 944노출에 5주간 클릭 0이었는데, 실제로 클릭을 만든 쿼리는 `screaming frog license free`(라이선스/무료판 의도)였고 **본문은 그 얘기를 단 한 줄도 안 하고 있었다.** 페이지를 새로 쓰거나 버리기 전에, GSC 쿼리 목록과 본문 H2를 나란히 놓고 **검색자가 묻는 것과 본문이 답하는 것이 같은지** 확인할 것. 이게 이번 주 유일한 조치였고, 지금까지 중 가장 근거가 명확한 작업이었다.
37. **(신규, 2026-08-24) 경쟁이 포화된 주제라도 "신규 글" 대신 "기존 글 보강"이 정답일 때가 있다.** `screaming frog free vs paid`는 대형 경쟁사가 이미 포화라 신규 글로는 안 뚫린다. 그런데 우리는 이미 그 클러스터에서 944노출·33.8위를 갖고 있었으므로, **별도 글을 쓰면 자기잠식이 되고 신규 페이지는 최소 1~2주 크롤 대기열에 앉는다.** 판단 기준: **해당 클러스터에서 이미 순위가 있으면 보강, 없으면 신규.**
38. **(신규, 2026-08-24) 신규 페이지는 발행 후 최소 1~2주 미크롤링 큐에 앉는다는 걸 전제로 우선순위를 짤 것.** 8/11 발행 글이 8/18에야 "발견됨-미색인"에 진입했고, 그 상태로 대기 중이다. **공격적 확장 전략은 유지하되(사용자 확정 지시), 주차별 우선순위에서는 "이미 색인돼 노출/클릭이 나오는 페이지 강화"가 "84번째 페이지 추가"보다 ROI가 명확히 높다.** 이건 전략 변경이 아니라 같은 전략 안에서의 순서 조정이다.

39. **(신규, 2026-08-25) 신규 툴 후보는 "항목 간 비교라야만 답이 나오는 문제"에서 먼저 찾을 것 — 우리가 반복해서 뚫은 유일한 각도다.** heading-structure-checker(페이지 간), frontmatter-checker(포스트 간), json-field-consistency(레코드 간) 세 개가 전부 이 패턴으로 채택됐다. 이유가 구조적이다: 경쟁 도구는 대부분 **항목 하나를 처리**하거나 **둘을 비교**하도록 만들어져 있어서, N개를 함께 봐야만 보이는 문제는 시장에 비어 있다. 반대로 "항목별 독립 처리"류(URL 길이, alt 텍스트, 리다이렉트 생성)는 배치도 no-upload도 이미 다 선점돼 있다. **후보를 떠올렸으면 먼저 물을 것: 이건 1개짜리를 N번 돌린 것인가, 아니면 N개를 함께 봐야만 나오는 답인가?** 전자면 십중팔구 포화다.
40. **(신규, 2026-08-25) 검증된 휴리스틱은 다른 도메인으로 그대로 옮길 것.** frontmatter에서 만든 "비슷한 두 이름이 한 항목에서 절대 같이 안 나오면 같은 필드" 판정을 JSON 페이로드에 그대로 적용해 `user_id`/`userId` 드리프트를 잡았다(`title`/`subtitle` 오탐 없음도 그대로 유지됨). 새 툴마다 판정 로직을 처음부터 짜지 말고 **기존 툴에서 이미 오탐까지 검증된 규칙이 있는지 먼저 볼 것.**

41. **(신규, 2026-08-31) "발견됨-미색인"은 내부링크로도 lastmod로도 해결되지 않는다 — 온사이트 레버는 소진됐으니 같은 실험을 반복하지 말 것.** 8/11 lastmod 전면 도입, 8/18 고노출 페이지 본문링크 8개 — **두 개입 모두 3주 이상 지나도록 18개 중 하나도 못 뺐다.** 특히 8/18은 대조군 설계까지 해뒀는데(`pdf-merge-compress`만 링크 없음) **링크를 심은 3개와 결과가 동일**해서 가설이 깨끗하게 기각됐다. 결정적으로, 미색인인 8/11 발행 글은 **순위 12.75위 페이지에서 본문 링크를 받고 있는데도** 색인이 안 되고, 8/19 발행분은 갓 만든 신규 툴에서만 링크를 받고도 색인됐다 — **링크 강도와 색인 여부가 아예 상관이 없다.** 남은 변수는 도메인 권위/외부 신호이며 이건 온사이트 작업으로 못 만든다. **다음 세션에서 "링크를 더 심어보자"는 판단이 들면 이 항목을 먼저 읽을 것.**
42. **(신규, 2026-08-31) 부정 결과는 반드시 그대로 기록할 것 — 특히 내가 직전 세션에 자신 있게 세운 가설이 깨졌을 때.** 8/18에 "출처 페이지의 크롤 빈도로 가중치를 줘야 한다"며 28항을 개정까지 했는데, 3주 뒤 데이터로 그 개입이 실패했다. 28항의 *분석 방법*(보일러플레이트로 지표가 평준화되지 않았는지 보라)은 여전히 유효하지만, 거기서 도출한 *처방*(본문링크를 심으면 크롤이 흐른다)은 틀렸다. **분석이 맞아도 처방이 맞는 건 아니라는 걸 분리해서 기록할 것.** 성공만 적으면 다음 세션이 같은 실험을 다시 한다.
...
- **A. 8/24 스크리밍프로그 + 8/31 CSV 보강 효과 확인(36항 검증).** 전자 3주차, 후자 2주차. **둘 다 "순위는 있는데 본문이 검색의도와 어긋난 페이지 보강" 방식이라, 효과가 나오면 현 국면의 표준 작업 유형으로 확정된다.** 안 나오면 36항도 처방으로서는 재검토 대상(42항 정신).
- **B. 8/25·9/01 발행분 색인 여부.** `json-field-consistency`/`which-api-fields-are-optional`(8/25), `package-json-version-checker`/`semver-range-drift-monorepo`(9/01). 8/19 발행분처럼 바로 색인되는지, 8/11 발행분처럼 대기열에 앉는지가 **신규 발행 ROI 판단의 핵심 데이터**.
- **C. 미색인 18개는 온사이트로 더 건드리지 말 것(41항).** 내부링크·lastmod 둘 다 실패했다. **같은 실험 재시도 금지.**
- **D. `chatgpt.com / ai-assistant` 3주 연속 여부.** 8/24·8/31 2주 연속. 3주면 llms.txt 강화 근거로 승격, 끊기면 노이즈.
- **E. JWT 클러스터 재검토 트리거.** 개별 클레임 쿼리(`jti jwt` 18.5위 등)가 더 오르거나 Bing 상위에서 클릭이 나오면 그때. 현재는 페이지가 내용을 다 다루고 있어 조치 불필요.
- **F. humans.txt 순위 하락(28.98→39.31) 관찰만.** 상업가치 0.
- **G. (사용자 판단 필요) Bing/IndexNow** — 11번 19항과 충돌하므로 임의 실행 안 함.
- **(수익화 항목은 9-1-1의 트래픽 조건을 실측으로 충족하기 전까지 이 목록에 올리지 말 것.)**

**★ 2026-08-31 세션 기준 다음 할 일**

- **A. 8/24 스크리밍프로그 보강 + 8/31 CSV 보강 효과 동시 확인.** 전자는 2주차(`screaming frog license free` 33.83위 → 20위권 오는지, 페이지 1061노출에 클릭 1 → 2 이상), 후자는 1주차(`excel-csv-number-mangling` 구글 29.2위 → 상승 여부, Bing 5.25위에서 클릭 발생 여부). **둘 다 36항 방식(순위는 있는데 본문이 검색의도와 어긋난 페이지 보강)이므로, 효과가 나오면 이 방식이 현 국면의 표준 작업 유형으로 확정된다.**
- **B. 8/25 발행 2페이지 색인 여부.** `json-field-consistency`, `which-api-fields-are-optional`. 8/19 발행분처럼 바로 색인되는지, 8/11 발행분처럼 대기열에 앉는지가 **신규 발행 ROI 판단의 핵심 데이터**다.
- **C. 미색인 18개는 온사이트로 더 건드리지 말 것(41항).** 내부링크·lastmod 둘 다 실패했다. 외부 신호(백링크·도메인 권위)가 붙기 전까지는 대기. **같은 실험을 재시도해 시간 쓰지 말 것.**
- **D. JWT 클러스터 재검토 트리거.** 개별 클레임 쿼리(`jti jwt` 18.5위, `jwt sub field` 25위, `jwt id` 31위)가 더 오르거나 Bing 상위(registered/public/private 2위)에서 클릭이 나오면 그때 보강 검토. **현재는 페이지가 이미 해당 내용을 다 다루고 있어 조치 불필요.**
- **E. `chatgpt.com / ai-assistant` 3주 연속 여부.** 8/24·8/31 2주 연속 1건씩. **3주 연속이면 llms.txt 클러스터 강화 근거로 승격**하고, 끊기면 노이즈로 처리.
- **F. humans.txt 순위 하락 관찰만.** 28.98→39.31로 처음 꺾임. 상업가치 0이라 조치는 안 하되, **도메인 권위 지표로 쓰던 용도가 유효한지**는 다음 스냅샷에서 재확인.
- **G. (사용자 판단 필요) Bing/IndexNow** — Bing 노출이 2배로 늘고 범용 bulk 쿼리에서 상위. 11번 19항과 충돌하므로 임의 실행 안 함.
- **(수익화 항목은 9-1-1의 트래픽 조건을 실측으로 충족하기 전까지 이 목록에 올리지 말 것.)**

**★ 2026-08-25 세션 기준 다음 할 일**

- **A. 8/24 스크리밍프로그 보강 효과 확인이 최우선.** `screaming frog license free`(33.8위)가 20위권으로 오는지, 페이지 클릭이 1→2 이상 되는지. 효과 있으면 **36항 방식(노출은 있는데 본문이 검색의도와 어긋난 페이지를 찾아 보강)을 다른 페이지에도 확대 적용**할 것 — 신규 페이지가 크롤 대기열에 앉는 현 상황에서 ROI가 가장 확실한 작업 유형이다.
- **B. 8/18 본문링크 8개 판정 — 이번이 진짜 판정 시점(3주 경과).** sitemap-generator(출처 858노출)·image-batch(174)·csv-tsv-converter(57)가 미크롤링에서 빠지는지. **pdf-merge-compress만 링크 없음 = 자연 대조군.**
- **C. 신규 페이지 크롤 대기 추적.** 8/19 발행 2개 + 8/25 발행 2개. **38항대로 최소 1~2주 대기를 전제로 볼 것.** 4개 다 큐에 앉아 있으면 "신규 발행보다 기존 강화" 우선순위가 확정된다.
- **D. `chatgpt.com / ai-assistant` 유입 재확인.** 8/24에 처음 1건. 다시 나오면 llms.txt 클러스터 강화 근거가 된다.
- **E. humans.txt 추적만**(60노출/28.98위, 5스냅샷 연속 상승, 상업가치 0).
- **F. (사용자 판단 필요) Bing/IndexNow** — Bing 클릭 2건, 순위 2~10위 유지. 11번 19항과 충돌하므로 임의 실행 안 함.
- **(수익화 항목은 9-1-1의 트래픽 조건을 실측으로 충족하기 전까지 이 목록에 올리지 말 것.)**

**★ 2026-08-24 세션 기준 다음 할 일**

- **A. 스크리밍프로그 클러스터가 이번 주 최우선 관찰 대상.** 8/24 보강 후 `screaming frog license free`(33.8위)가 20위권으로 올라오는지, 페이지 클릭이 1→2 이상 되는지. **이 클러스터는 노출 약 550건(499+22+20+6)으로 사이트 최대이면서 유일하게 새 클릭을 만든 곳** — 여기서 CTR이 붙으면 그게 첫 실질 성장 경로다. 효과 있으면 같은 방식(노출 있는데 본문이 검색의도와 어긋난 페이지 찾아 보강)을 다른 페이지에도 적용할 것.
- **B. 8/18 본문링크 8개 판정 — 다음 스냅샷이 진짜 판정 시점(2~3주 경과).** sitemap-generator(출처 858노출)·image-batch(174)·csv-tsv-converter(57)가 미크롤링에서 빠지는지. **pdf-merge-compress만 링크를 못 심었으므로 자연 대조군.**
- **C. 신규 3페이지 색인 추적.** sitemap-lastmod-git-commit-date(8/11 발행 → 8/18 큐 진입), frontmatter-checker·markdown-front-matter-mistakes(8/19 발행). **셋 다 큐에 앉아 있으면 "신규 발행 = 최소 2주 대기"가 확정되므로, 그때는 주차별 우선순위를 확실히 '기존 페이지 강화'로 기울일 것**(전략 변경이 아니라 순서 조정).
- **D. RSS/n8n 클러스터 — 판단 유보로 전환.** `zapier rss`는 4주 연속 29.33위 완전 고정이라 죽은 것처럼 보였으나, **이번 주 rss-generator가 클릭 3→4로 늘었고 `rss feed builder free`라는 신규 클릭 쿼리도 나옴.** 8/18에 "다음에도 안 움직이면 투자 중단"이라고 적었지만 **툴 페이지 쪽은 살아있으므로 중단 대상에서 제외**하고, 블로그 글(`rss-for-automation`, 124노출 4주째 완전 정체)만 정체로 볼 것.
- **E. `chatgpt.com / ai-assistant` 유입 재확인.** 이번 주 처음 1건 발생. llms.txt 투자 효과일 수 있으나 1건이라 신호 아님. **다음 스냅샷에도 나오면 llms.txt 클러스터 강화 근거가 된다.**
- **F. humans.txt는 추적만.** 60노출/28.98위, 5스냅샷 연속 상승. 상업가치 0이므로 콘텐츠 투자 금지, 1페이지 진입 시 도메인 권위 상승 증거로만 활용.
- **G. (사용자 판단 필요) Bing/IndexNow** — Bing 클릭이 2건으로 늘었고 순위 2~10위 유지. 11번 19항과 충돌하므로 임의 실행 안 함.
- **(수익화 항목은 9-1-1의 트래픽 조건을 실측으로 충족하기 전까지 이 목록에 올리지 말 것.)**
