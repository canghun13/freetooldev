...
마지막 업데이트: 2026-08-18 (GSC + **Bing 웹마스터** 스냅샷 분석 + 조치 세션 — 최대 발견 2건: (1) **지난주 "내부링크 문제 아님" 결론이 틀렸음을 자체 반증** — 링크 개수가 아니라 **출처 페이지의 크롤 빈도**로 봐야 했고, 최대 노출 페이지들이 본문 링크 0~1개짜리 막다른 골목이었음. 고노출 페이지 8곳에서 미크롤링 페이지로 문맥 링크 8개 신설. (2) **Bing은 우리를 2~8위에 놓음**(Google은 50~70위), Google이 한 번도 안 읽은 페이지까지 색인 중. 그 외 신규 글 1개(sitemap lastmod × git commit date, 우리 1차 경험 기반), sitemap-validator에 **lastmod 균일성/미래날짜/부분누락 검사 신설**(경쟁 밸리데이터에 없는 기능). 전체 81파일 = sitemap url 81 = llms.txt 81.)
