...
마지막 업데이트: 2026-08-25 (**신규 제작 세션** — 사용자가 "키워드 폭넓게, GSC 밖에서, 새 클러스터 환영" 지시. 우리 클러스터 밖 후보 5개 검토 → **4개 정직 기각, 1개 채택**. 신규 툴 `tools/json-field-consistency.html`(28번째) + 페어링 블로그 `blog/which-api-fields-are-optional.html`. **채택 근거는 "JSON 비교 도구가 전부 2개 문서 diff뿐이고 N개 레코드 일관성 분석이 시장에 없다"** — 한 경쟁사 블로그가 bash 루프로 10개를 1번과 하나씩 비교하라고 안내할 만큼 통증은 문서화돼 있는데 도구가 없었음. 기존 `api-schema-drift-explained`·`json-validator`에서 신규 페이지로 본문 링크 2개 신설(30항). 전체 85파일 = sitemap 85 = llms.txt 85, 푸터 85/85 정합.)
