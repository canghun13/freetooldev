...
마지막 업데이트: 2026-08-31 (**주간 데이터 세션 — 조치 1건, 신규 0건.** 클릭 10 유지(8/22 이후 9일 신규 0), 노출 3970→4798(+21%). **최대 발견은 부정 결과 2건**: (1) **8/18 내부링크 실험 3주차 실패** — 고노출 페이지에서 본문 링크를 심은 3개 툴이 전부 미색인 그대로이고 링크 못 심은 대조군과 결과 동일 → "내부링크로 크롤예산이 흘러간다" 가설 기각(41항). (2) **신규 페이지 색인 여부가 링크로 설명이 안 됨** — 8/19 발행분 2개는 색인돼 노출까지 나오는데, 순위 12.75위 페이지에서 링크를 받는 8/11 발행분은 여전히 미색인. 조치는 `blog/excel-csv-number-mangling.html` 1건(구글 29.2위·Bing 5.25위인데 Bing 쿼리가 본문에 없는 메커니즘을 묻고 있어 15자리 경계·복구 가능성 판별을 추가, 786→1344단어). Bing 노출 2배(39→80)에 `bulk utility tool` 5.4위 등 **범용 "bulk 도구" 쿼리에서 상위** 확인. 전체 85파일 = sitemap 85 = llms.txt 85.)
...
**대기 중인 신규 툴 후보**: 없음 (15차에서 5개 검토 후 4개 기각·1개 채택으로 소진). 다음 세션에서도 "배치 차별화" 우선, 막히면 "no-upload 차별화"(8차), 그마저 없으면 "경쟁사가 전부 단일 항목만 처리하는 헤드키워드"(10차) 순으로 확인하고, **전부 막히면 억지로 만들지 말고 정직하게 기각 보고할 것**(11차에서 확립된 원칙).
