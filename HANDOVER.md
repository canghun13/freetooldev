...
마지막 업데이트: 2026-07-31 (새 세션 — 사용자가 공유한 외부 피드백(launch 플랫폼 댓글, "Selçuk": "PDF 합치기+압축 기능이 있었으면 좋겠다, 여러 사이트 오가는 게 번거롭다")을 계기로 **완전 신규 카테고리 "PDF" 신설**. 처음엔 1차버전 범위(merge만 / +safe / +aggressive 전부)를 물었으나 사용자가 "1차버전 말고 완성까지 다 해달라"고 해서 3개 모드(None/Safe/Aggressive) 전부 한 번에 구현. `tools/pdf-merge-compress.html` 신규 툴 + `blog/why-merged-pdf-is-bigger.html` 신규 블로그 페어링. pdf-lib(cdnjs)+pdfjs-dist(jsDelivr, cdnjs는 구버전 고정이라 예외적으로 다른 CDN 사용) 조합. jsdom+node-canvas로 실제 렌더링까지 포함한 3개 모드 전부 end-to-end 테스트 완료. 전체 사이트 80개 파일 카운트 정합성/링크/고아페이지/태그균형 전부 재검증 통과.)
...
## 3. 사이트 구조 (전체 파일, 2026-07-25 세션 기준 78개)
