...
마지막 업데이트: 2026-07-27 (새 세션 — GSC/GA 7/27 스냅샷 분석. 신규 툴/블로그는 만들지 않음(강한 신규 키워드 신호 없음, 억지로 안 만듦). 대신 처음으로 실제 클릭 전환이 확인된 `blog/sitemap-static-sites.html`을 웹 검색 기반 문제해결형 섹션(GitHub Pages Jekyll `.nojekyll` 404 원인/해법 + Netlify·Vercel 배포 디렉토리 불일치 + FAQ 4개)으로 보강. AdSense 신청 타이밍은 이번에도 시기상조로 재확인. Coverage 4개 스냅샷 연속 완전 동일(이상無).)
