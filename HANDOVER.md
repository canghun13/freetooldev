...
마지막 업데이트: 2026-07-27 2차 (같은 날 새 세션 — 사용자가 공유한 실제 사용자 피드백(launch 플랫폼 댓글, "Recep": "배치 처리가 최고였는데 리사이즈+압축 조합을 프리셋으로 저장했으면 좋겠다") 반영. `tools/image-batch.html`에 **저장된 프리셋(Saved presets)** 기능 신설 — 리사이즈모드/크기/포맷/퀄리티 5개 값을 이름 붙여 저장→드롭다운으로 재적용→삭제. localStorage 기반(서버 전송 없음, 사이트의 "브라우저에서만 처리" 정체성과 일치). jsdom으로 저장/덮어쓰기/불러오기/삭제/빈 이름 거부 5개 시나리오 전부 동작 검증. 사이트 전체에서 localStorage를 쓰는 최초 사례.)
